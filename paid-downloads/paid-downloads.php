...
<?php
/*
Plugin Name: Paid Downloads پارسی webgate
...
class paiddownloads_class {
	...
	var $zarinpalwg_currency_list = array("تومان");
	...
	function __construct() {
		...
		$this->currency_list = array_unique(array_merge($this->paypal_currency_list, $this->alertpay_currency_list, $this->zarinpalwg_currency_list));
		...
		$this->options = array (
			...
			"enable_zarinpalwg" =>"off",
			"zarinpalwg_merchant" => "",
			"zarinpalwg_currency" => $zarinpalwg_currency_list[0],
...
//		}
	...
	function check_settings() {
		$errors = array();
		if ($this->options['enable_alertpay'] != "on" && $this->options['enable_paypal'] != "on" && $this->options['enable_zarinpalwg'] != "on") $errors[] = __('Select at least one payment method', 'paiddownloads');
		...
		if ($this->options['enable_zarinpalwg'] == "on") {
			if (strlen($this->options['zarinpalwg_merchant']) < 16) { $errors[] = "کدبازرگانی (Merchant) زرین‌پال ارزش ندارد.";}
		}
						...
						<div class="postbox">
							...
							<div class="inside">
								<table class="paiddownloads_useroptions">
									<tr>
										<th>فعال‌کردن</th>
										<td><input type="checkbox" id="paiddownloads_enable_zarinpalwg" name="paiddownloads_enable_zarinpalwg" '.($this->options['enable_zarinpalwg'] == "on" ? 'checked="checked"' : '').'> انجام تراکنش‌ها با زرین‌پال<br /><em>برای اعمال پرداخت‌ها از زرین‌پال این گزینه را فعال کنید</em></td>
									</tr>
									<tr>
										<th>کدبازرگانی:</th>
		...
		print('								
			...
			<script type="text/javascript">
				function paiddownloads_supportedmethods() {
					...
					var zarinpalwg_currencies = new Array("'.implode('", "', $this->zarinpalwg_currency_list).'");
					var currency = jQuery("#paiddownloads_currency").val();
					var supported = "";
					if (jQuery.inArray(currency, paypal_currencies) >= 0) supported = "PayPal, ";
					if (jQuery.inArrat(currency, zarinpalwg_currencies) >= 0) supported = "زرین‌پال, ";
		...
		if (!empty($_POST['ak_action'])) {
			switch($_POST['ak_action']) {
				case 'paiddownloads_update_settings':
					...
					if (isset($_POST["paiddownloads_enable_zarinpalwg"])) $this->options['enable_zarinpalwg'] = "on";
					else $this->options['enable_zarinpalwg'] = "off";
...
$zp->soap_defencoding = 'UTF-8';
				...
				$payer = $_COOKIE["zarinpalwg_email"];
				$currency = " tomans";
				$first_name = $payer;
				$last_name = $payer;
				if ($res == 100) {
					...
					if (intval($file_details["id"]) == 0) $payment_status = "unrecognized";
						...
						if (floatval($amount) < floatval($file_details["price"])) $payment_status = "unrecognized";
									...
									if ($res == 100) $payment_status = "success";
				if ($res <> 100) $payment_status = "unrecognized";
					$sql = "INSERT INTO ".$wpdb->prefix."pd_transactions (
						file_id , payer_name, payer_email, gross, currency, payment_status, transaction_type, details, created) VALUES (
						'".intval($item_number)."',
					...
					$mail_headers .= "From: ".$this->options['from_name']." <".$this->options['from_email'].">\r\n";
					$mail_headers .= "X-Mailer: PHP/".phpversion()."\r\n";
					wp_mail($this->options['seller_email'], "یک پرداخت زرین پال کامل انجام شد", $body, $mail_headers);
					echo '
			...
			$_SESSIN["zarinpalwg_amount"] = "";
			$_COOKIE["zarinpalwg_email"] = "";
			exit;
	...
	function shortcode_handler($_atts) {
		...
		if ($this->check_settings() === true)
		{
			...
			if (intval($sales["sales"]) < $file_details["available_copies"] || $file_details["available_copies"] == 0)
			{
				...
				if ($this->options['enable_zarinpalwg'] == "on") $methods++;
				...
				$_SESSION["zarinpalwg_amount"] = $file_details["price"] ;
				$button .= '
				<script type="text/javascript">
					var active_'.$button_id.' = "'.($this->options['enable_paypal'] == "on" ? 'paypal_'.$button_id : ($this->options['enable_alertpay'] == "on" ? 'alertpay_'.$button_id : 'zarinpalwg_'.$button_id)).'";
					function paiddownloads_'.$button_id.'() {
						...
						if (active_'.$button_id.' == "zarinpalwg_'.$button_id.'") {
							...
							setCookie("zarinpalwg_email", paiddownloads_email, 1);
							
							setCookie("zarinpalwg_title", "'.$file_details["title"].'", 1);
							setCookie("zarinpalwg_id", '.$file_details["id"].', 1);
						}
...
$zp->soap_defencoding = 'UTF-8';
					$payID = $zp->call('PaymentRequest', array(
						array(
									...
									'Email' 		=> '' ,
									'Mobile' 		=> '' ,
									'CallbackURL' 	=> get_bloginfo("wpurl")."/?zp_verify=1"
									)
					...
					if($payID['Status'] == 100)
					{
						...
						'.($methods > 1 ? '
						<script type="text/javascript">
							if (jQuery("#method_izarinpalwg_'.$button_id.'")) {
